    ...
    if db is not None:
        ...
        if (len(master_password)) == 1:
            ...
            if master_password[0][0] == password:
                user_id = execute_query(db, f"SELECT user_id FROM users WHERE username = '{username}';")[0][0]
...
def sign_up(username, password, email):
    ...
    if db is not None:
        ...
        username_check = execute_query(db, f"SELECT COUNT(*) FROM users WHERE username = '{username}';")[0][0]

        # get number of users with the same email
        email_check = execute_query(db, f"SELECT COUNT(*) FROM users WHERE email = '{email}';")[0][0]
...
def add_password(user_id, domain, password, account_name="", url=""):
    ...
    if db is not None:
        ...
        matching_pswrd = execute_query(db, f"SELECT COUNT(*) FROM passwords WHERE domain = '{domain}' AND "
                                      f"account_name = '{account_name}';")[0][0]
